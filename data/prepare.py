...
from utils.errors import DataError, handle_errors, validate_directory_writable
...
def load_dataset_with_fallback(dataset_name: str, max_sequences: int):
    ...
    try:
        ...
        raise DataError(
            f"Failed to load dataset '{dataset_name}': {e}. Check dataset name or install required datasets library."
        )
...
def process_sequences(dataset, tokenizer: GPT2Tokenizer, config: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for i, example in enumerate(tqdm(dataset)):
        ...
        try:
            ...
            processed_sequences.append({"text": text, "tokens": tokens, "length": len(tokens)})
            processed_count += 1
    ...
    logger.info(
        f"Final stats: {processed_count} kept, {skipped_short} too short, {skipped_duplicates} duplicates, {skipped_tokenization} tokenization errors"
    )
...
def split_and_save_data(sequences: List[Dict[str, Any]], config: Dict[str, Any]) -> None:
    ...
    logger.info(
        f"Split ratios - Train: {config['train_split']}, Val: {config['val_split']}, Test: {1 - config['train_split'] - config['val_split']}"
    )
    ...
    for data, filename, description in files_to_save:
        if data:
            ...
            logger.info(
                f"  {description} saved in {file_time:.2f} seconds ({os.path.getsize(path) / 1024 / 1024:.1f} MB)"
            )
