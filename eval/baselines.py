...
def evaluate_baselines(
    ...
    for baseline in baselines:
        ...
        for ratio in target_ratios:
            ...
            baseline_results[f'ratio_{ratio}'] = {
...
def load_corpus_for_baselines(data_path: str) -> Tuple[List[List[str]], Dict[str, Any]]:
    ...
    for item in data:
        if 'tokens' in item:
            # Token IDs - convert to strings
            tokens = [str(t) for t in item['tokens']]
        elif 'text' in item:
            # Raw text - simple tokenization
...
def test_baselines():
    ...
    test_sequences = [
        ...
        ["this", "is", "a", "longer", "sequence", "with", "more", "tokens", "for", "testing", "purposes"]
    ]
...
def demonstrate_baseline_comparison():
    """Demonstrate how baselines compare on a single example."""
    tokens = ["the", "extraordinarily", "quick", "brown", "fox", "jumps", "over", "the", "lazy", "dog"]
    target_ratio = 0.5
    ...
    print(f"Target ratio: {target_ratio} (keep {int(len(tokens) * target_ratio)} tokens)")
    print("-" * 60)
    ...
    for name, baseline in baselines:
        ...
        print(f"{name:10}: {' '.join(kept_tokens)}")
    
    print("-" * 60)
    print("Notice how different strategies keep different tokens!")
