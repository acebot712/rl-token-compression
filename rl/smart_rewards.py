...
class InformationTheoreticReward:
    ...
    def compute_reward(
        ...
        entropy = -(mask_probs * torch.log(mask_probs + 1e-8) + 
                   (1 - mask_probs) * torch.log(1 - mask_probs + 1e-8)).mean(dim=1)
...
def test_reward_functions():
    ...
    logger.info(f"Simple reward - mean: {simple_results['reward'].mean():.3f}, std: {simple_results['reward'].std():.3f}")
