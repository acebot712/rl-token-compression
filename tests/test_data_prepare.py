...
def prepare_test_data(output_dir: str, num_samples: int = 10, max_length: int = 100) -> None:
    """
    ...
    for i, text in enumerate(sample_texts[:num_samples]):
        # Tokenize
        tokens = tokenizer.encode(text, max_length=max_length, truncation=True, return_tensors="pt")[0].tolist()

        processed_sequences.append({"tokens": tokens, "length": len(tokens), "source": "test_sample"})
