...
from utils.errors import DeviceError, ModelError
...
class DistributedJointTrainer(SimpleJointTrainer):
    ...
    def __init__(self, rank: int, world_size: int, device_type: str = "cuda", **kwargs):
        """
    ...
    def setup_models(self, tokenizer, reconstructor):
        ...
        self.reconstructor = DistributedDataParallel(
            self.reconstructor,
            device_ids=[self.rank] if self.device_obj.type == "cuda" else None,
        ...
        if self.is_main_process:
            ...
            print(
                f"✓ Models wrapped with DDP - Policy: {policy_params:,} params, Reconstructor: {recon_params:,} params"
            )
...
def create_distributed_trainer(
    config: Dict[str, Any], train_data: List[List[int]], reconstructor, tokenizer
...
def launch_distributed_training():
    ...
    Example usage:
        torchrun --nproc_per_node=2 training/distributed.py --config configs/training/default.json
    """
    import sys
