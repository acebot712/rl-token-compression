...
class SimpleJointTrainer:
    ...
    def train_step(self, batch_sequences: List[List[int]]) -> Dict[str, float]:
        """Simple training step - let PyTorch handle memory naturally."""
        ...
        for i in range(0, len(batch_sequences), self.micro_batch_size):
            ...
            try:
                # Forward pass - let PyTorch handle memory naturally
        ...
        for epoch in range(max_epochs):
            ...
            for i in range(0, len(train_data), batch_size):
                ...
                if batch_num % 10 == 1:  # Log every 10th batch
                    logger.info(f"Processing batch {batch_num}/{total_batches} (sequences {i}-{min(i+batch_size, len(train_data))})")
                ...
                if self.step % 100 == 0:
                    ...
                    logger.info(f"Step {self.step}/{total_steps}: "
