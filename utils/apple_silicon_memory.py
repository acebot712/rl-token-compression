...
class UnifiedMemoryManager:
    ...
    def __init__(self, device: str = "auto", memory_target_gb: float = 32.0):
        ...
        logger.info("UnifiedMemoryManager initialized for Apple Silicon")
        logger.info(f"  Total unified memory: {self.total_memory_gb:.1f}GB")
        logger.info(
            f"  Target memory usage: {memory_target_gb:.1f}GB ({(memory_target_gb / self.total_memory_gb * 100):.1f}%)"
        )
    ...
    def suggest_gradient_accumulation(self, desired_batch_size: int) -> Tuple[int, int]:
        ...
        logger.info(
            f"Suggested gradient accumulation: micro_batch={micro_batch_size}, "
