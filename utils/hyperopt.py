...
class SimpleHyperparameterOptimizer:
    ...
    def sample_hyperparameters(self, trial_id: int) -> Dict[str, Any]:
        ...
        params['learning_rate_policy'] = np.exp(np.random.uniform(
            np.log(self.search_space.learning_rate_policy_min),
            np.log(self.search_space.learning_rate_policy_max)
        ))
        
        params['learning_rate_reconstructor'] = np.exp(np.random.uniform(
        ...
        params['gradient_accumulation_steps'] = np.random.choice(
    ...
    def _run_sequential_optimization(
        ...
        for trial_id in range(self.max_trials):
            ...
            # Update best result
            if result.error is None and (self.best_result is None or 
                                       result.best_loss < self.best_result.best_loss):
                improvement = (self.best_result.best_loss - result.best_loss 
