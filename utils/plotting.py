...
def create_comparison_bar_plot(
    ...
    for bar, value in zip(bars, values):
        ...
        ax.text(
            ...
            height + (error_vals[bars.index(bar)] if error_vals else 0),
            f"{value:.3f}",
            ha="center",
            va="bottom",
            fontweight="bold",
...
def create_scatter_plot(
    ...
    # Create scatter plot
    colors = [
        BASELINE_COLORS.get(label, COLORS["baseline"]) if labels else COLORS["primary"]
        for label in (labels or [""] * len(x_data))
    ]

    ax.scatter(x_data, y_data, c=colors, alpha=0.7, s=100, edgecolors="black", linewidth=0.5)
...
def create_research_dashboard(
    ...
    ax1.bar(methods, values, color=colors, alpha=0.8, edgecolor="black", linewidth=0.5)
